...
ChatLogic::ChatLogic()
{
    //// TASK 5: DONE create instance of chatbot in function LoadAnswerGraphFromFile(std::string)
    ////
...
ChatLogic::~ChatLogic()
{
    ...
    // delete all nodes, smart pointer no need to delete explicitly
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // find_if return value: Iterator to the first element satisfying the condition or last if no such element is found
                        // &node unique_ptr reference, not copy
                        // using const
                        ...
                        if (newNode == _nodes.end())
                        {
                            ...
                            //std::unique_ptr<GraphNode> node_temp = std::make_unique<GraphNode>(id);
                            //_nodes.push_back(std::move(node_temp));
    ...
    // TASK 5: DONE
    // create local instance chatbot and move to graph root node
    // remain pointer _chatbot in ChatLogic
